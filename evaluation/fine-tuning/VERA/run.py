...
class DeclarativeDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        sourcess_tok = [self.tokenizer.batch_encode_plus(
            sources,
            return_tensors='pt', padding='max_length', truncation='longest_first',
...
class Trainer:
    ...
    def logitss(self, sourcess_input_ids, sourcess_attention_mask):
        ...
        flattened_sourcess_attention_mask = sourcess_attention_mask.view(-1,
                                                                         sourcess_attention_mask.size(-1))  # (B * C, L)
        #print("input ids",flattened_sourcess_input_ids)
        # print("masks",flattened_sourcess_attention_mask)
        outputs = self.model(
        ...
        #print("last hidden", last_hidden_state)
    ...
    def loss(self, batch, split):
        ...
        if self.args.loss_fn == 'margin':
            ...
            for i in range(1, logitss.size(1)):
                # print("loss append", loss_fn(logitss[:, 0], logitss[:, i],
                #                              torch.ones(logitss.size(0), dtype=torch.long, device=device)))
                losses.append(
                    loss_fn(logitss[:, 0], logitss[:, i], torch.ones(logitss.size(0), dtype=torch.long, device=device)))
        ...
        loss = loss_scoring
        # print("Loss before contrastive", loss)
        ...
        if self.args.contrastive_loss_type != 0:
            ...
            if self.args.contrastive_loss_type == 1:
                ...
                _logitss = cos_fn(flattened_hidden.unsqueeze(-1),
                                  flattened_all_hidden.t().unsqueeze(0))  # (B * C, num_gpus * B * C)
                _logitss = _logitss / self.args.contrastive_loss_temp
                loss_fn = torch.nn.CrossEntropyLoss(reduction='none')
                offset = accelerator.process_index * _logitss.size(0)
                losses = []
    ...
    def valid(self, step):
        ...
        eval_acc = stats['eval/mc/acc_unweighted_seen'] if 'eval/mc/acc_unweighted_seen' in stats else stats[
            'eval/mc/acc_unweighted_all']
    ...
    def eval(self, step):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(
                    tqdm(self.eval_dataloader) if accelerator.is_main_process else self.eval_dataloader):
        ...
        for (task_ix, first_is_correct, logits, scores) in zip(task_ixs, first_is_corrects, logitss, scoress):
            ...
            if FORMAT_BY_TASK[task] == 'mc':
                ...
                for i in range(1, logits.size(0)):
                    if logits[i] > -1e8:
                        booleans.append(
                            {'task': task, 'logit': logits[i].item(), 'score': scores[i].item(), 'label': 0.0})
            else:  # boolean task
                booleans.append({'task': task, 'logit': logits[0].item(), 'score': scores[0].item(),
                                 'label': first_is_correct.float().item()})
        if not self.args.nosave and accelerator.is_main_process:
...
def get_args():
    ...
    # train
    parser.add_argument('--train_tasks', type=str,
                        default='openbookqa,arc_easy,arc_hard,ai2_science_elementary,ai2_science_middle,commonsenseqa,qasc,physical_iqa,social_iqa,winogrande_xl,com2sense_paired,sciq,quarel,quartz,cycic_mc,comve_a,csqa2,symkd_anno,gengen_anno')
    parser.add_argument('--valid_tasks', type=str,
                        default='openbookqa,arc_easy,arc_hard,ai2_science_elementary,ai2_science_middle,commonsenseqa,qasc,physical_iqa,social_iqa,winogrande_xl,com2sense_paired,sciq,quarel,quartz,cycic_mc,comve_a,csqa2,symkd_anno,gengen_anno')
    parser.add_argument('--eval_tasks', type=str,
                        default='openbookqa,arc_easy,arc_hard,ai2_science_elementary,ai2_science_middle,commonsenseqa,qasc,physical_iqa,social_iqa,winogrande_xl,com2sense_paired,sciq,quarel,quartz,cycic_mc,comve_a,csqa2,symkd_anno,gengen_anno,wsc273,copa,numersense,prost,spatial_cs,swag,hellaswag,codah,story_cloze_test,alphanli,strategyqa,creak,rainier_anno')
...
def main():
    ...
    log_info(f'Loading metaphysical_event_data ...')
    ...
    if args.mode == 'train':
        ...
        optimizer = torch.optim.AdamW(chain(model.parameters(), linear.parameters()), lr=args.lr)
