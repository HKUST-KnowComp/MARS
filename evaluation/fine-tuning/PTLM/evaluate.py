...
def evaluate(tokenizer, model, device, loader, test_threshold=0.5, model_type="roberta"):
    ...
    # # print the predicted class distribution
    # print(predicted_class.tolist().count(0), predicted_class.tolist().count(1))
    # # plot the predicted scores into a histogram
    # plt.hist(predicted_scores.tolist(), bins=100)
    # plt.savefig("predicted_scores.png")

    return roc_auc_score(labels.tolist(), (predicted_scores).tolist()), f1_score(labels.tolist(), (
        predicted_class).tolist(), average='macro'), accuracy_score(labels.tolist(), (predicted_class).tolist()), len(
        labels)
