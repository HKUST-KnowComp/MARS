...
class DataTrainingArguments:
    """
    Arguments pertaining to what metaphysical_event_data we are going to input our model for training and eval.
    ...
    lora_rank: Optional[int] = field(
        default=32, metadata={"help": "the lora rank"}
    )
...
def main():
    ...
    # load three csv dataset at ../../../data_with_fixed_boundary/train, dev, test.csv
    train_dataset = load_dataset("csv", data_files="DATA_DIRECTORY")
    # train_dataset['train'] = train_dataset['train'].select(random.sample(range(len(train_dataset['train'])), 50))
    eval_dataset = load_dataset("csv", data_files="DATA_DIRECTORY")
    # eval_dataset['train'] = eval_dataset['train'].select(random.sample(range(len(eval_dataset['train'])), 50))
    predict_dataset = load_dataset("csv", data_files="DATA_DIRECTORY")
    # predict_dataset['train'] = predict_dataset['train'].select(random.sample(range(len(predict_dataset['train'])), 50))
    ...
    if model_args.load_from_pretrain:
        if ("falcon" in model_args.model_name_or_path or "Llama-2" in model_args.model_name_or_path or
                "gpt" in model_args.model_name_or_path or "Llama-3" in model_args.model_name_or_path):
            kwargs = {}
            ...
            kwargs = {"target_modules": ["q_proj", "o_proj", "k_proj", "v_proj", "gate_proj", "up_proj", "down_proj"]}
        else:
    ...
    if is_main_process(local_rank):
        ...
        logger.info("metaphysical_event_data size: train {}, valid {}, test {}".format(
            len(train_dataset['train']), len(eval_dataset['train']), len(predict_dataset['train'])))
    ...
    def preprocess_function(examples):
        ...
        #     examples['event_replaced'][i] = "If " + examples['event_replaced'][i]
        #     examples['inference'][i] = "then " + examples['inference'][i]
        return tokenizer(
            examples['event_after_transition'],
            examples['inference'],
            padding=padding,
    ...
    if training_args.do_train:
        ...
        if is_main_process(local_rank):
            logger.info(f"Few-shot experiments with train metaphysical_event_data size: {len(train_dataset['train'])}")
            for index in random.sample(range(len(train_dataset['train'])), 3):
                logger.info(f"Sample {index} of the training set: {train_dataset['train'][index]}.")
                logger.info(tokenizer.convert_ids_to_tokens(train_dataset['train'][index]["input_ids"]))
    ...
    if training_args.do_eval:
        ...
        if is_main_process(local_rank):
            for index in random.sample(range(len(eval_dataset['train'])), 3):
                logger.info(f"Sample {index} of the eval set: {eval_dataset['train'][index]}.")
                logger.info(tokenizer.convert_ids_to_tokens(eval_dataset['train'][index]["input_ids"]))
    ...
    if training_args.do_eval:
        ...
        eval_results = trainer.predict(test_dataset=eval_dataset['train'], metric_key_prefix="valid")
        ...
        metrics["valid_samples"] = len(eval_dataset['train'])

        trainer.log_metrics("valid", metrics)
        trainer.save_metrics("valid", metrics)

        range_idx = np.arange(len(eval_dataset['train'])).reshape(-1, 1)
    ...
    if training_args.do_predict:
        logger.info("*** Test ***")
        eval_results = trainer.predict(test_dataset=predict_dataset['train'], metric_key_prefix="test")
        metrics, label_ids, pred_prob = eval_results.metrics, eval_results.label_ids, eval_results.predictions
        pred_prob = pred_prob[0] if isinstance(pred_prob, tuple) else pred_prob
        metrics["test_samples"] = len(predict_dataset['train'])

        trainer.log_metrics("test", metrics)
        trainer.save_metrics("test", metrics)

        range_idx = np.arange(len(predict_dataset['train'])).reshape(-1, 1)
        pred_label = np.argmax(pred_prob, axis=-1).reshape(-1, 1)
